...
function updateProcessTable() {
    ...
    processes.forEach((process, index) => {
        ...
        const powerCell = row.insertCell(3);
        let powerColor = "";
...
function createGanttChart(ganttData) {
    ...
    if (!document.getElementById('gantt-chart-styles')) {
...
function showGanttChart() {
    ...
    // Create Gantt chart data from completed processes
    const ganttData = [];
    let currentTime = 0;
    ...
    completedProcesses.forEach(process => {
        ...
        if (process.firstExecutionTime !== -1) {
            ganttData.push({
                pid: process.pid,
                startTime: process.firstExecutionTime,
                endTime: process.firstExecutionTime + process.burstTime
            });
        }
    });
